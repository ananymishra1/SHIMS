...
def patch_llm_chat(monkeypatch):
    ...
    Also patches _ollama_chat_stream to the streaming-call-shaped equivalent
    (model, messages, tools, on_delta) -> dict, since ollama is in
    STREAMING_WAVE_PROVIDERS and the wave router now calls that instead of
    _llm_chat directly. The fake never calls on_delta, matching how a
    mocked _llm_chat response arrives as one complete blob with no
    incremental callback — so these tests keep exercising the same
    router -> tool-wave -> final-answer logic without a real network call.
    """
    def _patch(fn):
        monkeypatch.setattr("shared.agent_loop._llm_chat", fn)

        async def _fake_stream(model, messages, tools, on_delta):
            result, success, _, _ = await fn(None, model, messages, tools)
            return result

        monkeypatch.setattr("shared.agent_loop._ollama_chat_stream", _fake_stream)
    return _patch
