...
class LMStudioProvider(AIProvider):
    """LM Studio local OpenAI-compatible server provider.

    LM Studio exposes an OpenAI-compatible API on its default port (1234).
    This provider uses the same transport as OpenAI-compatible clouds but
    targets the local LM Studio instance, enabling GPU acceleration
    (Vulkan/Metal/CUDA) that Ollama's bundled llama.cpp may not support.
    """
    async def complete(self, prompt: str, system: str = '', tools: Optional[list[dict[str, Any]]] = None, model: Optional[str] = None) -> AIResult:
        used_model = model or settings.lmstudio_model or 'google/gemma-4-e4b'
        ...
        try:
            async with httpx.AsyncClient(timeout=300) as client:
                res = await client.post(f'{base_url}/v1/chat/completions', json=payload, headers=headers)
                res.raise_for_status()
                data = res.json()
            text = data.get('choices', [{}])[0].get('message', {}).get('content', '')
            return AIResult(
                text=text.strip() or 'LM Studio returned an empty response.',
                provider='lmstudio',
                model=used_model,
                route='lmstudio',
                raw=data,
            ...
            fallback = await FallbackProvider().complete(prompt, system, tools, model=model)
            fallback.text = f'LM Studio unavailable ({exc}). {fallback.text}'
            fallback.ok = False
            fallback.error = str(exc)
            fallback.route = 'lmstudio:fallback'
            return fallback
