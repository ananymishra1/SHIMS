...
SEARCH_TRIGGERS = [
    "search", "internet", "web", "look up", "lookup", "browse", "google",
    "latest", "current", "recent", "today", "news", "verify online",
    "online", "patent", "patents", "cas number", "cas no", "regulation",
...
def plan_search_query(text: str, *, web_mode: bool = False, force_search: bool = False, max_variants: int = 4) -> SearchQueryPlan:
    ...
    should_search = force_search or _has_trigger(original)
    if web_mode and not should_search:
        low = original.lower()
        should_search = len(low.split()) >= 4 and any(v in low for v in ["research", "verify", "find", "source", "cite", "compare", "check"])
