...
# v4 agent state + reasoning (LM Studio-style timing)
...
async def _lmstudio_chat_raw(model: str, messages: list[dict[str, Any]], tools: list[dict[str, Any]], timeout: float = 120.0) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        ...
        if r.status_code == 400 and "model reloaded" in r.text.lower():
...
class FinalFieldStreamer:
    """Incrementally reveals a `"final": "..."` JSON string value from a
    growing text buffer, as it arrives.

    The wave-router prompt (agent_wave._WAVE_SYSTEM) asks the model for
    strict JSON — ``{"reasoning": "...", "wave": [...], "final": "..."}`` —
    so a turn that needs no more tools ends with the answer sitting inside
    that JSON's "final" field. The streaming chat functions below already
    detect "JSON mode" (content starting with ``{`` or a code fence) and
    buffer it silently so raw tool-call JSON never flickers into the UI —
    but that means a direct answer wrapped in the required JSON contract
    never streamed either, even though the underlying HTTP call is a real
    token stream. This class lets those functions forward just the "final"
    field's text as it's revealed, while the surrounding JSON structure
    (reasoning/wave) stays buffered exactly like before.

    Purely additive: the complete accumulated text is still parsed by
    agent_wave._parse_wave exactly as before once the stream ends, so
    tool-call correctness is unaffected — this class only decides what to
    show the user *while waiting*, never what the agent loop acts on.
    """
    ...
    def feed(self, delta: str) -> str:
        ...
        if not self._streaming:
            m = _FINAL_FIELD_KEY_RE.search(self._buffer)
            if not m:
                return ""
        ...
        while scan < len(text):
            ...
            if ch == "\\":
                if scan + 1 >= len(text):
                    break  # escape sequence split across chunks — wait for more
                scan += 2
                continue
            if ch == '"':
                segment = text[i:scan]
                self._cursor = scan + 1
                self._done = True
                return self._unescape(segment)
            scan += 1
        boundary = self._safe_boundary(text, i, scan)
        segment = text[i:boundary]
        self._cursor = boundary
        return self._unescape(segment)

    @staticmethod
    def _safe_boundary(text: str, start: int, end: int) -> int:
        """Never emit a dangling backslash or a partially-arrived \\uXXXX
        escape that a later chunk would complete."""
        if end > start and text[end - 1] == "\\":
            return end - 1
        tail_start = max(start, end - 6)
        idx = text.rfind("\\", tail_start, end)
        if idx != -1 and idx + 1 < end and text[idx + 1] == "u" and (end - idx - 2) < 4:
            return idx
        return end
...
async def _stream_chat_for_provider(
    provider: str, model: str, messages: list[dict[str, Any]],
    tools: list[dict[str, Any]], on_delta: Callable[[str], Any],
) -> dict[str, Any]:
    """Dispatch to the streaming chat function for providers known to
    support it reliably (ollama, lmstudio, anthropic). Kimi/DeepSeek/Qwen/
    OpenAI are deliberately excluded here — final-answer synthesis below
    already avoids streaming them due to fragile SSE parsing, so the
    higher-stakes wave-routing call keeps the same conservative choice."""
    if provider == "ollama":
                        ...
                        if delta.get("type") == "text_delta":
                            ...
                            if text:
                                ...
                                if mode is None:
                                    ...
                                    emitted = final_streamer.feed(text)
                                    if emitted:
                                        await on_delta(emitted)
...
async def run_agent_loop(
    *,
    ...
    # v4: Initialize structured turn state and reasoning stream (LM Studio-style timing)
    state: AgentState = new_agent_state(
        session_id=session_id or "",
        user_query=message,
        messages=list(messages),
        system_prompt=convo[0].get("content", "") if convo and convo[0].get("role") == "system" else "",
        provider=provider,
        model=model,
    )
    reasoning = ReasoningStream(state)

    async for ev in reasoning.emit("agent", "Agentic mode activated. Analyzing request..."):
        yield ev
    ...
    if provider != "ollama":
        ...
        try:
            ...
            async for ev in reasoning.model_thought(
                "plan", "Explicit plan generated.", plan_started, model=model, provider=provider
            ):
                yield ev
        except Exception as exc:
            async for ev in reasoning.model_thought(
                "plan", f"Plan generation failed: {str(exc)[:160]}. Proceeding without explicit plan.",
                plan_started, model=model, provider=provider
            ):
                yield ev
    else:
        async for ev in reasoning.emit("agent", "Using inline reasoning (plan generation skipped for local model speed)."):
            yield ev

    async for ev in reasoning.emit("agent", "Router is planning tool waves."):
        yield ev
    ...
    # Wave-router streaming: when the router/executor decides no more tools
    # are needed, its "final" answer normally only reaches the user after the
    # complete JSON response is parsed. For providers with a reliable stream
    # (see STREAMING_WAVE_PROVIDERS), _router_chat/_executor_chat push the
    # "final" field's text here as FinalFieldStreamer reveals it; the wave
    # loop below drains this while awaiting plan_wave, same pattern as the
    # fast lane. wave_streamed_text tracks what's been shown so the loop can
    # commit to it as the answer once the call resolves — mirroring how the
    # fast lane never re-runs a full fallback once real content has streamed.
    wave_pending: list[str] = []
    wave_streamed_text = ""

    async def _wave_on_delta(t: str) -> None:
        nonlocal wave_streamed_text
        wave_streamed_text += t
        wave_pending.append(t)
    ...
    async def _router_chat(wave_messages: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        try:
            if router_provider in STREAMING_WAVE_PROVIDERS:
                result = await asyncio.wait_for(
                    _stream_chat_for_provider(router_provider, router_model, wave_messages, specs, _wave_on_delta),
                    timeout=15.0,
                )
    ...
    async def _executor_chat(wave_messages: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        try:
            if provider in STREAMING_WAVE_PROVIDERS:
                result = await asyncio.wait_for(
                    _stream_chat_for_provider(provider, model, wave_messages, specs, _wave_on_delta),
                    timeout=25.0,
                )
                if result.get("content") is not None or result.get("tool_calls"):
                    return result
    ...
    # Heartbeat events queued by the wave-planning heartbeat callback and
    # drained into the stream alongside streamed tokens. plan_wave's
    # heartbeat machinery has existed since v3, but its callback was a no-op
    # — so during a long silent model think the NDJSON stream emitted
    # nothing, proxies could cut the connection, and the UI looked hung.
    wave_events: list[dict[str, Any]] = []

    async def _drain_wave_pending() -> AsyncGenerator[dict[str, Any], None]:
        while wave_events:
            yield wave_events.pop(0)
        while wave_pending:
    ...
    for wave in range(1, max_waves + 1):
        ...
        async for ev in reasoning.emit("plan", f"Wave {wave}: routing parallel tool calls..."):
            yield ev

        # v3: Plan wave with router (fast) — heartbeat keeps the stream alive
        # while the model thinks (the wave loop drains wave_events as it polls).
        ...
        def _heartbeat():
            ...
            wave_events.append({"type": "status", "heartbeat": True,
                                "content": f"Wave {wave}: model still planning... ({elapsed}s)"})

        wave_streamed_text = ""
        wave_pending.clear()
        wave_events.clear()
        wave_convo = convo + [{"role": "user", "content": f"ORIGINAL REQUEST (never forget): {message}"}]
        wave_plan_started = time.perf_counter()
        heartbeat_every = _heartbeat_seconds()
        router_failed = False
        try:
            plan_task = asyncio.create_task(
                agent_wave.plan_wave(wave_convo, _router_chat, valid_names, on_heartbeat=_heartbeat, heartbeat_interval=heartbeat_every)
            )
            while not plan_task.done():
                async for ev in _drain_wave_pending():
                    yield ev
                await asyncio.sleep(0.02)
            async for ev in _drain_wave_pending():
                yield ev
            calls, final = await plan_task
        except Exception:
            router_failed = True
            async for ev in reasoning.model_thought(
                "agent", "Router model unavailable, falling back to executor for wave planning.",
                wave_plan_started, model=router_model, provider=router_provider
            ):
                yield ev
            try:
                plan_task = asyncio.create_task(
                    agent_wave.plan_wave(wave_convo, _executor_chat, valid_names, on_heartbeat=_heartbeat, heartbeat_interval=heartbeat_every)
                )
                while not plan_task.done():
                    async for ev in _drain_wave_pending():
                        yield ev
                ...
                async for ev in reasoning.model_thought(
                    "agent", f"Wave planning failed: {str(exc)[:160]}",
                    wave_plan_started, model=model, provider=provider
                ):
                    yield ev
        ...
        if not router_failed:
            used_router = router_model if router_provider == provider else f"{router_provider}/{router_model}"
            async for ev in reasoning.model_thought(
                "plan", f"Wave {wave} routed by {used_router}.",
                wave_plan_started, model=router_model, provider=router_provider
            ):
                yield ev

        if wave_streamed_text.strip():
            # Real text already reached the user for this wave (the router/
            # executor call streamed a "final" answer as it was generated).
            # Commit to it now rather than risking a second, different
            # answer appearing after it — same principle as the fast lane:
            # once something has streamed, never re-synthesize from scratch.
            answer = (final or wave_streamed_text).strip()
        ...
        if result.stop and result.stop_reason == "all_failed":
            ...
            async for ev in reasoning.emit("agent", "All tools in this wave failed. Stopping."):
                yield ev
            break
        ...
        if not any_success:
            ...
            async for ev in reasoning.emit("agent", "No successful tool calls in this wave. Stopping."):
                yield ev
            break
    ...
    if not answer and stop_reason not in {"approval", "planning_failed"}:
        ...
        async for ev in reasoning.emit("agent", "Synthesizing final answer from tool results."):
            yield ev
        if used_tools:
        ...
        async def _pump_synthesis(ftask: "asyncio.Task") -> AsyncGenerator[dict[str, Any], None]:
            """Drain streamed tokens while the synthesis task runs, emitting a
            keepalive status whenever the model sits silent (prompt processing
            on big local models can be 10-40s with zero output)."""
            hb_every = _heartbeat_seconds(provider)
            ...
            while not ftask.done():
                if fin_pending:
                    ...
                    hb_last = time.perf_counter()
                elif (time.perf_counter() - hb_last) >= hb_every:
                    yield {"type": "status", "heartbeat": True,
                           "content": f"Synthesizing final answer... ({int(time.perf_counter() - synth_started)}s)"}
                    hb_last = time.perf_counter()
                await asyncio.sleep(0.02)
            while fin_pending:
                yield {"type": "token", "content": fin_pending.pop(0)}
        ...
        try:
            if provider == "anthropic":
                ...
                # Must be checked BEFORE _is_openai_compatible — lmstudio is in
                # that set, so with the old order this streaming branch was
                # unreachable and LM Studio synthesis silently fell into the
            ...
            # Some models (especially local ones) return {"final": "..."} instead of plain text.
            # Extract the actual answer so the user never sees raw JSON.
            final_answer = _final_from_text({"content": answer}, valid_names)
            if final_answer:
                answer = final_answer
            answer_streamed = fin_streamed or not _is_openai_compatible(provider)
            async for ev in reasoning.model_thought(
                "synthesis", "Final answer synthesized.", synth_started, model=model, provider=provider
            ):
                yield ev
        except Exception as exc:
            import logging
            ...
            async for ev in reasoning.model_thought(
                "synthesis", f"Final synthesis failed: {str(exc)[:160]}", synth_started, model=model, provider=provider
            ):
                yield ev
