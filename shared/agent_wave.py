...
def _parse_wave(content: str, valid_names: set[str]) -> tuple[list[WaveCall], str | None]:
    ...
    Supports both the v3 wave format:
      {"wave": [{"tool": "...", "args": {...}}], "final": "..."}
    And the legacy JSON-action fallback:
      {"tool": "...", "args": {...}} or {"final": "..."}

    Uses shared.ai.extract_json_maybe to tolerate markdown fences,
    trailing prose, and minor formatting issues.
    """
    ...
    if "wave" in data:
        ...
        for item in data.get("wave", []) or []:
            ...
            # Normalize: some models emit underscores (web_search) instead of dots (web.search)
            normalized_name = name.replace("_", ".")
            if normalized_name not in valid_names and name not in valid_names:
                continue
            use_name = normalized_name if normalized_name in valid_names else name
            args = item.get("args") or item.get("arguments") or {}
            purpose = item.get("purpose") or item.get("reason") or ""
            calls.append(WaveCall(use_name, args, purpose))
        return calls, None

    # Legacy JSON-action fallback: {"tool": "...", "args": {...}}
    final = data.get("final")
    if final and isinstance(final, str) and final.strip():
        return [], final.strip()
    ...
    # Normalize: some models emit underscores (web_search) instead of dots (web.search)
    normalized_name = name.replace("_", ".")
    if normalized_name in valid_names:
        args = data.get("args") or data.get("arguments") or {}
        purpose = data.get("purpose") or ""
        return [WaveCall(normalized_name, args, purpose)], None
    # Also try the original name without normalization
    if name in valid_names:
        args = data.get("args") or data.get("arguments") or {}
        purpose = data.get("purpose") or ""
        return [WaveCall(name, args, purpose)], None
