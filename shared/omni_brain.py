...
def _connect() -> Generator[sqlite3.Connection, None, None]:
    """Open the brain database and ensure it is closed when done.

    sqlite3.Connection's context manager only commits/rolls back; it does
    NOT close the connection. This wrapper closes it on exit to prevent
    connection leaks during long-running sessions.
    """
...
def remember_turn(
    ...
    *,
    ...
    if len(user_text or "") > 40 or len(assistant_text or "") > 60:
        ...
        # The source_uri must be unique PER TURN. ingest_knowledge upserts by
        # (source_type, source_uri), so the old shared "session:<id>" uri made
        # every new turn DELETE the previous turn's archive — only the last
        # exchange of a session ever survived in RAG, which silently capped
        # conversational memory at one turn. A per-turn uri makes the archive
        # append-only, so the whole conversation stays recallable forever
        # (this is the storage substrate for unlimited virtual context).
        ingest_knowledge(
            f"Conversation {session_id} {datetime.now().strftime('%Y-%m-%d %H:%M')}",
            digest[:4000],
            source_type="conversation",
            source_uri=f"session:{session_id}:turn:{int(_now() * 1000)}",
            tags=["conversation", f"session:{session_id}", agent or "supervisor", route or "chat"],
            importance=0.65,
        )
    return result


def recall_conversation(session_id: str, query: str, *, limit: int = 4, max_rows: int = 800) -> list[dict[str, Any]]:
    """Recall the archived turns of THIS session most relevant to ``query``.

    This is the retrieval half of unlimited virtual context: recent turns stay
    verbatim in the prompt window, older turns live in the append-only archive
    written by remember_turn, and this function pulls back the few archived
    exchanges that matter for the current message. Designed to be fast enough
    for the chat fast lane: one indexed sqlite scan over this session's chunks
    plus keyword scoring, blended with semantic vector hits when the embedding
    layer is warm (both fail soft).
    """
    q = _clean(query)
    ...
    def _add(title: str, text: str, score: float) -> None:
        key = text[:120]
        if not text or key in seen:
            return
        seen.add(key)
        hits.append({"title": title, "text": text, "score": round(float(score), 4)})

    try:
        with _connect() as con:
            rows = con.execute(
                # LIKE 'session:<id>%' matches both the new per-turn uris and
                # any legacy single-uri archive left from before the fix.
                "SELECT title, chunk_text, importance, updated_at FROM knowledge_chunks "
                "WHERE source_type='conversation' AND source_uri LIKE ? ORDER BY updated_at DESC LIMIT ?",
                (f"session:{session_id}%", max_rows),
            ).fetchall()
        for row in rows:
            score = _score(q, f"{row['title']} {row['chunk_text']}", weight=float(row["importance"] or 0.65), created_at=row["updated_at"])
            if score > 0:
                _add(row["title"], row["chunk_text"], score)
    except Exception:
        pass

    # Semantic hits, filtered to this session's conversation archive.
    try:
        for v in _search_vectors(q, limit=max(limit * 3, 12)):
            sim = float(v.get("similarity") or 0.0)
            meta = v.get("metadata") or {}
            uri = str(meta.get("source_uri") or "")
            if sim < 0.45 or meta.get("source_type") != "conversation":
                continue
            if not uri.startswith(f"session:{session_id}"):
                continue
            _add(meta.get("title") or "Conversation", _clean(v.get("text_content") or ""), sim * 12.0)
    except Exception:
        pass
