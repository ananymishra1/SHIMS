...
from shared.amd_acceleration import get_amd_accelerator, amd_gpu_status, amuse_video_status, comfy_ui_status
...
_settings: dict[str, Any] = {
    "voice": {
        ...
        "command_cooldown_seconds": float(os.getenv("SHIMS_VOICE_COMMAND_COOLDOWN_SECONDS", "1.2")),
        "silence_timeout_seconds": float(os.getenv("SHIMS_VOICE_SILENCE_TIMEOUT_SECONDS", "1.0")),
...
# _collect_cloud_stream below), so simple chat on a cloud provider can also
# use the fast lane instead of blocking for a full completion.
CLOUD_STREAMING_PROVIDERS = {"openai", "anthropic", "gemini", "kimi", "deepseek", "qwen"}
...
LESSONS_CACHE_TTL = 30.0


def _cached_lessons_text() -> str:
    """load_daily_lessons_text() does blocking file I/O + JSON parsing; it's
    called on every single chat turn via _system_prompt(). The lessons file
    only changes a few times a day, so a short cache avoids stalling the
    event loop with disk reads on every message."""
    global _LESSONS_CACHE
    ts, text = _LESSONS_CACHE
    now = time.time()
    if (now - ts) < LESSONS_CACHE_TTL:
        return text
    text = load_daily_lessons_text()
    _LESSONS_CACHE = (now, text)
    return text
...
class ChatRequest(BaseModel):
    ...
    agent_mode: bool = False  # Default off: agent loop adds latency and model overrides
...
class VoiceConfigRequest(BaseModel):
    ...
    command_cooldown_seconds: float = float(os.getenv("SHIMS_VOICE_COMMAND_COOLDOWN_SECONDS", "1.2"))
    silence_timeout_seconds: float = float(os.getenv("SHIMS_VOICE_SILENCE_TIMEOUT_SECONDS", "1.0"))
...
# Short-TTL cache for local model discovery (Ollama /api/tags, LM Studio
# /api/v1/models). These get queried multiple times on a single chat turn
# (routing, agent-tool model selection, fast-lane resolution) — without
# caching, every turn pays 2-4 real HTTP round trips just to answer "what
# models are installed" before any LLM call even starts. A few seconds of
...
async def _cached_model_list(key: str, fetch: Callable[[], Awaitable[list[dict[str, Any]]]]) -> list[dict[str, Any]]:
    ...
    if cached and (time.time() - cached[0]) < MODEL_LIST_CACHE_TTL:
        return cached[1]
    data = await fetch()
    # Don't cache empty results — an empty list usually means the provider was
    # briefly unreachable, and we want the very next call to retry for real
    # rather than parroting "offline" for the rest of the TTL window.
    if data:
        _MODEL_LIST_CACHE[key] = (time.time(), data)
    return data


def _invalidate_model_list_cache(key: str | None = None) -> None:
    """Call after pulling/downloading a model or changing a provider host/key
    so the new model is visible immediately instead of waiting out the TTL."""
    if key:
...
async def _ollama_models_raw(timeout: float = 2.5) -> list[dict[str, Any]]:
    ...
    return await _cached_model_list("ollama", _fetch)
...
async def _lmstudio_models_raw(timeout: float = 2.5) -> list[dict[str, Any]]:
    ...
    return await _cached_model_list("lmstudio", _fetch)
...
def _is_wake_ping(text: str) -> bool:
    """True only for wake-word/attention checks that deserve an instant local
    ack ("suno", "are you there") — NOT real greetings like "hi"/"hello",
    which should get a genuine LLM reply."""
    t = _normalize_text(_strip_wake(text))
    return t in {"", "sun rahe ho", "suno", "are you there", "listen"}
...
async def _search_duckduckgo(query: str, max_results: int) -> list[dict[str, str]]:
    """Use the DuckDuckGo search library for reliable no-key search.
    Falls back to HTML scraping if the library returns no results.
    For news-related queries, uses the news() method first."""
    if not _settings.get("web", {}).get("duckduckgo_fallback", True):
        return []
    
    # Detect news queries
    is_news_query = any(kw in query.lower() for kw in ["news", "latest", "breaking", "today", "recent"])
    ...
    # Try the DDGS library first — news method for news queries
    if is_news_query:
        try:
            with DDGS() as ddgs:
                results = ddgs.news(query, max_results=max(1, min(max_results, 10)), region="wt-wt")
    ...
    # Try the DDGS text search with retries — DuckDuckGo can transiently return empty results
    ...
    # Fallback: HTML scraping via DuckDuckGo lite endpoint
    try:
...
async def _make_plan(req: ChatRequest) -> TurnPlan:
    ...
    # Only wake pings ("suno", "are you there") and voice greetings get the
    # instant local ack. Typed greetings ("hi", "hello") go to the LLM so the
    # user gets a real reply instead of a canned line.
...
#
# _openai_chat / _anthropic_chat / _gemini_chat / _openai_compatible_chat
# above all block until the provider finishes the FULL completion before any
# text reaches the user — for a multi-paragraph answer that's several extra
# seconds of silence even though the API itself streams tokens. These
# counterparts stream real tokens as they arrive, so a cloud-provider chat
# gets the same instant time-to-first-token as the local fast lane.
# ---------------------------------------------------------------------------


async def _openai_compatible_stream_lines(
    url: str, headers: dict[str, str], payload: dict[str, Any]
) -> AsyncGenerator[tuple[str, bool], None]:
    """Shared SSE parser for the OpenAI chat/completions wire format
    (delta.content / delta.reasoning_content) — used by OpenAI, Kimi,
    DeepSeek, and Qwen, which are all OpenAI-compatible on this endpoint."""
    read_timeout = 240.0
    async with httpx.AsyncClient(timeout=httpx.Timeout(300.0, read=read_timeout)) as client:
...
async def _openai_chat_stream(model: str, messages: list[dict[str, str]], *, max_tokens: int | None = None) -> AsyncGenerator[tuple[str, bool], None]:
    api_key = _clean_secret(os.getenv("OPENAI_API_KEY"))
    if not api_key:
        yield ("OpenAI is selected but OPENAI_API_KEY is not configured. Select an installed Ollama/LM Studio model or add the key in Settings.", False)
        return
    payload = {"model": model, "messages": messages, "temperature": 0.2, "max_tokens": max_tokens or 32000, "stream": True}
    headers = {"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"}
    try:
        async for chunk in _openai_compatible_stream_lines("https://api.openai.com/v1/chat/completions", headers, payload):
            yield chunk
    except httpx.HTTPStatusError as exc:
        status = exc.response.status_code if exc.response is not None else 0
        if status == 401:
            yield ("OpenAI rejected this API key with 401 Unauthorized. Update OPENAI_API_KEY in Settings.", False)
        else:
            yield (f"OpenAI API error ({status}). Check OPENAI_API_KEY, billing, and that model `{model}` is available.", False)
    except httpx.TimeoutException:
        yield ("OpenAI request timed out. Try again or use a local model.", False)
    except httpx.RequestError as exc:
        yield (f"OpenAI connection failed: {str(exc)[:200]}.", False)


async def _openai_compatible_provider_chat_stream(
    provider: str, model: str, messages: list[dict[str, str]], *, max_tokens: int | None = None
) -> AsyncGenerator[tuple[str, bool], None]:
    """Streaming counterpart to _openai_compatible_chat (Kimi/DeepSeek/Qwen)."""
    env = PROVIDER_ENV.get(provider)
    api_key = _clean_secret(os.getenv(env or ""))
    if not api_key:
        yield (f"{provider.title()} model `{model}` needs {env or 'an API key'}. Add it in Settings or choose Local/Ollama.", False)
        return
    base_urls = {
        "kimi": os.getenv("KIMI_BASE_URL", "https://api.moonshot.ai/v1").rstrip("/"),
        "deepseek": os.getenv("DEEPSEEK_BASE_URL", "https://api.deepseek.com/v1").rstrip("/"),
    ...
    base = base_urls.get(provider)
    if not base:
        yield (await _cloud_placeholder(provider, model), False)
        return
    candidates = [model]
    if provider == "kimi":
        from shared.kimi_model_helper import normalize_kimi_model, kimi_fallback_chain
    ...
    for attempt_model in candidates:
        temperature = 1.0 if provider == "kimi" and isinstance(attempt_model, str) and attempt_model.startswith("kimi-k2") else 0.2
        payload = {"model": attempt_model, "messages": messages, "temperature": temperature, "stream": True, "max_tokens": max_tokens or 32000}
        got_any = False
        try:
            async for chunk in _openai_compatible_stream_lines(f"{base}/chat/completions", headers, payload):
                got_any = True
                yield chunk
            if got_any:
                return
            last_error = f"{attempt_model}: empty stream"
        except httpx.HTTPStatusError as exc:
            status = exc.response.status_code if exc.response is not None else 0
            if status == 401:
                yield (f"{provider.title()} rejected this API key with 401 Unauthorized. Update {env} in Settings.", False)
                return
            if status == 404 and provider == "kimi" and attempt_model != candidates[-1]:
                last_error = f"Kimi model `{attempt_model}` not found (404)."
                continue
            yield (f"{provider.title()} API error ({status}). Check {env}, billing, and model `{attempt_model}`.", False)
            return
        except httpx.TimeoutException:
            yield (f"{provider.title()} request timed out. Try again or use a local model.", False)
            return
        except httpx.RequestError as exc:
            yield (f"{provider.title()} connection failed: {str(exc)[:200]}.", False)
            return
    yield (f"{provider.title()} exhausted all fallback models. {last_error}", False)


async def _anthropic_chat_stream(model: str, messages: list[dict[str, str]], *, max_tokens: int | None = None) -> AsyncGenerator[tuple[str, bool], None]:
    api_key = _clean_secret(os.getenv("ANTHROPIC_API_KEY"))
    if not api_key:
        yield (f"Claude model `{model}` needs ANTHROPIC_API_KEY. Select an installed Ollama/LM Studio model or add a valid key in Settings.", False)
        return
    user_msgs = [{"role": m["role"], "content": m["content"]} for m in messages if m["role"] in {"user", "assistant"}][-50:]
    payload = {"model": model, "max_tokens": max_tokens or settings.max_output_tokens, "temperature": 0.2, "system": SYSTEM_PROMPT, "messages": user_msgs, "stream": True}
    headers = {"x-api-key": api_key, "anthropic-version": "2023-06-01", "content-type": "application/json"}
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(300.0, read=240.0)) as client:
            async with client.stream("POST", "https://api.anthropic.com/v1/messages", headers=headers, json=payload) as r:
                if r.status_code == 401:
                    yield ("Anthropic rejected this API key with 401 Unauthorized. Choose an installed Ollama/LM Studio model or paste a valid Anthropic key in Settings.", False)
                    return
                r.raise_for_status()
                async for line in r.aiter_lines():
                    if not line or not line.startswith("data: "):
                    ...
                    if obj.get("type") == "content_block_delta":
                        ...
                        yield (f"Anthropic error: {err.get('message', 'unknown error')}", False)
    except httpx.HTTPStatusError as exc:
        status = exc.response.status_code if exc.response is not None else 0
        yield (f"Anthropic API error ({status}). Check ANTHROPIC_API_KEY, billing, and that model `{model}` is available.", False)
    except httpx.TimeoutException:
        yield ("Anthropic request timed out. Try again or use a local model.", False)
    except httpx.RequestError as exc:
        yield (f"Anthropic connection failed: {str(exc)[:200]}.", False)


async def _gemini_chat_stream(model: str, messages: list[dict[str, str]], *, max_tokens: int | None = None) -> AsyncGenerator[tuple[str, bool], None]:
    api_key = _clean_secret(os.getenv("GEMINI_API_KEY"))
    if not api_key:
        yield (f"Gemini model `{model}` needs GEMINI_API_KEY. Add it in Settings or choose Local/Ollama.", False)
        return
    ...
    payload: dict[str, Any] = {"contents": contents or [{"role": "user", "parts": [{"text": ""}]}], "generationConfig": generation_config}
    if system_parts:
        payload["system_instruction"] = {"parts": system_parts}
    ...
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(300.0, read=240.0)) as client:
            async with client.stream("POST", url, headers=headers, params={"alt": "sse"}, json=payload) as r:
                r.raise_for_status()
                async for line in r.aiter_lines():
                    if not line or not line.startswith("data: "):
                        continue
        ...
        status = exc.response.status_code if exc.response is not None else 0
        yield (f"Gemini API error ({status}). Check GEMINI_API_KEY, billing, and that model `{model}` is available.", False)
    except httpx.TimeoutException:
        yield ("Gemini request timed out. Try again or use a local model.", False)
    except httpx.RequestError as exc:
...
def _cloud_stream_for(provider: str, model: str, messages: list[dict[str, str]], *, max_tokens: int | None = None):
    ...
    if provider == "anthropic":
        return _anthropic_chat_stream(model, messages, max_tokens=max_tokens)
    if provider == "gemini":
...
async def _collect_cloud_stream(
    provider: str,
    model: str,
    messages: list[dict[str, str]],
    *,
    ...
    """Collect tokens from a cloud-provider stream, calling on_delta/on_reasoning
    as they arrive. Mirrors _collect_lmstudio_stream/_collect_ollama_stream so
    the fast lane can treat local and cloud providers identically."""
    gen = _cloud_stream_for(provider, model, messages, max_tokens=max_tokens)
    if gen is None:
        raise RuntimeError(f"No streaming implementation for provider '{provider}'")
    answer = ""
    got_first = False
    agen = gen.__aiter__()
    while True:
...
async def _comfyui_image(prompt: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        return {"ok": False, "provider": "comfyui", "error": result.get("error") or "ComfyUI returned no image"}
    except Exception as exc:
        return {"ok": False, "provider": "comfyui", "error": str(exc)[:240]}
...
def _image_provider_plan(backend: str) -> list[tuple[str, Callable[[str], Awaitable[dict[str, Any] | None]]]]:
    ...
    if backend in {"comfyui", "comfy"}:
        return [("comfyui", providers["comfyui"])]
...
async def _create_video(prompt: str) -> dict[str, Any]:
    ...
    errors: list[str] = []
    # AMD AMUSE local video model (highest priority if installed)
    if backend in {"auto", "amuse", "amd"}:
        from shared.amd_acceleration import generate_amuse_video
        filename = _safe_name(prompt or "video", "mp4")
        path = VIDEO_DIR / filename
        result = await generate_amuse_video(prompt, path)
        if result.get("ok"):
            ext = Path(result["path"]).suffix.lstrip(".")
            url = f"/media/files/video/{Path(result['path']).name}"
            out = {
                "ok": True,
                "provider": "amuse",
                "type": "video",
                "kind": "video",
                "title": prompt[:80] or "Generated video",
            ...
            return _attach_ledger(out, Path(result["path"]), "video")
        if result.get("error"):
            errors.append(f"amuse: {result['error']}")
...
def _agentic_intent(text: str) -> bool:
    """Return True only when the message clearly needs local tools.

    Simple chat questions should stay on the fast direct-LLM path.
    Agent mode is reserved for explicit slash commands or patterns that
    involve code, files, shell commands, web search, desktop actions, etc.
    """
    raw = (text or "").strip()
    if not raw:
        return False
    ...
    if low in ("hi", "hello", "hey", "yo", "sup", "hola", "namaste"):
        return False
    if low.startswith(_AGENT_SLASH):
        return True
    return bool(_AGENTIC_PATTERNS.search(raw))
...
def _fast_chat_eligible(req: ChatRequest) -> bool:
    """Return True if this request can use the fast direct-LLM lane.

    This is intentionally synchronous and cheap: it only checks message intent.
    Model availability is resolved inside the stream, and any failure there
    raises so the caller falls back to the full pipeline.

    req.agent_mode is deliberately NOT checked: the UI historically sent
    agent_mode=true on every turn, which pushed simple chat ("hi", "how are
    you?") through the heavyweight agent loop. Messages that actually need
    tools are excluded by the intent checks below and still reach the agent
    loop through the full pipeline.
    """
    text = (req.message or "").strip()
    if not text or text.startswith("/"):
        return False
    # yes/no/haan must reach the approval router, not the LLM.
    if _approval_decision_from_text(text) is not None:
        return False
    if _agentic_intent(text) or _detect_tool_intent(text):
        return False
    # Wake pings and voice greetings keep their instant local ack.
    if _is_wake_ping(text) or ((req.source or "") == "voice" and _is_empty_greeting(text)):
        return False
    # Turns that genuinely want web data keep the search route.
    if _detect_search_intent(text, web_mode=bool(getattr(req, "web_mode", False))):
        return False
    provider = (req.provider or "auto").strip().lower()
    # Cloud providers (openai/anthropic/gemini/kimi/deepseek/qwen) get the
    # fast lane too: they have real streaming implementations below, so
    # simple chat on a cloud model skips RAG/plan overhead and streams
    # tokens live instead of blocking for a full completion.
    return provider in {"", "auto", "local", "lmstudio", "ollama"} or provider in CLOUD_STREAMING_PROVIDERS


def _stream_heartbeat_seconds(provider: str = "") -> float:
    """How long a chat stream may stay silent before a keepalive status is
...
# fire-and-forget task with no strong reference can be garbage-collected
# before it ever runs (verified live: background conversation archiving
# silently vanished this way). Keep tasks alive until they finish.
_background_tasks: set[asyncio.Task] = set()


def _spawn_background(coro) -> None:
    """Schedule fire-and-forget work with a strong reference so it survives GC,
    and log any failure — silent background death is how memory/archiving bugs
    stay invisible for months."""
    def _done(task: asyncio.Task) -> None:
        ...
        exc = task.exception()
        if exc is not None:
            log_event("background.task_error", route="background", provider="local", model="",
                      ok=False, message=str(exc)[:220])
            import sys as _sys
            print(f"[background-task-error] {exc!r}", file=_sys.stderr)
...
async def _fast_chat_stream(req: ChatRequest) -> AsyncGenerator[bytes, None]:
    """Fast lane for simple chat: no plan, no brain RAG, no agent loop."""
    started = time.perf_counter()
    session_id = req.session_id or str(uuid.uuid4())
    # Voice turns still need duplicate/silence suppression here because the
    # fast lane bypasses _make_plan (which normally runs _guard_duplicate).
    if (req.source or "") == "voice" and _guard_duplicate(session_id, req.message, req.source):
    ...
    if provider in {"", "auto", "local"}:
        provider, auto_model = await _local_default_provider_model()
        model = model or auto_model
    elif provider in CLOUD_STREAMING_PROVIDERS:
        # Never let a high-sensitivity message reach an explicitly-chosen
        # cloud provider — mirrors the same guard _resolve_provider_model
        # applies in the full pipeline (privacy-guard-high-explicit-override),
        # just enforced before dispatch here instead of after routing.
        from shared.privacy_guard import classify_sensitivity
        if classify_sensitivity(req.message) == "high":
            provider, model = await _local_default_provider_model()
        else:
    ...
    # in the brain's append-only conversation archive (written in the
    # background below) and is recalled here when relevant. The recall is
    # strictly time-budgeted so a long memory never slows the reply: if the
    ...
    if req.conversation_mode and len((req.message or "").split()) >= 3 and (req.source or "").lower() != "voice":
        try:
            hits = await asyncio.wait_for(
                ...
                timeout=float(os.getenv("SHIMS_CONTEXT_RECALL_TIMEOUT", "0.25")),
            )
            recent_text = " ".join(str(m.get("content", ""))[:400] for m in history[-50:])
            fresh = [h for h in hits if h.get("text") and h["text"][:100] not in recent_text]
            if fresh:
                recall_hits = len(fresh)
    ...
    async def on_reasoning(delta: str) -> None:
        ...
        # Emit reasoning in small chunks so the UI shows "thinking" activity.
        if len(reasoning_buffer) >= 80 or "\n" in delta:
            pending.append(_jsonl({"type": "thought", "stage": "reasoning", "content": reasoning_buffer[:400]}))
    ...
    try:
        if provider == "lmstudio":
            ...
            collect_task = asyncio.create_task(_collect_cloud_stream(
                ...
                on_reasoning=on_reasoning, first_token_timeout=20.0,
            ))
        else:
            return

        # Pump with heartbeat: big local models can sit silent for 10-40s
        # processing the prompt before the first token. Emit a keepalive
        # status while nothing else is flowing so proxies never cut the
        # stream and the UI shows the turn is alive, not hung.
        ...
        while not collect_task.done():
            if pending:
                while pending:
                    yield pending.pop(0)
                last_emit = time.perf_counter()
            elif (time.perf_counter() - last_emit) >= heartbeat_every:
                yield _jsonl({"type": "status", "heartbeat": True,
                              "content": f"Model is thinking... ({int(time.perf_counter() - started)}s)"})
                last_emit = time.perf_counter()
        ...
        if not answer.strip():
            yield _jsonl({"type": "token", "content": "I am connected but received an empty response from the model."})
        if req.conversation_mode:
            ...
            # the recent window (or after a server restart wipes _sessions).
            if answer.strip():
        ...
        if answer.strip():
            ...
            # re-running the whole pipeline (which would duplicate the answer).
            log_event("turn.fast_path_interrupted", route=f"{provider}-fast-path", provider=provider, model=model,
                      latency_ms=(time.perf_counter() - started) * 1000, ok=False,
                      message=req.message, metadata={"session_id": session_id, "error": str(exc)[:200]})
            yield _jsonl({"type": "done", "session_id": session_id, "model": model, "provider": provider,
                          "route": f"{provider}-fast-path-interrupted"})
            return
        log_event("turn.fast_path_fallback", route=f"{provider}-fast-path", provider=provider, model=model,
                  latency_ms=(time.perf_counter() - started) * 1000, ok=False,
                  message=req.message, metadata={"session_id": session_id, "error": str(exc)[:200]})
        raise
    ...
    if plan.provider == "ollama":
        ...
        if stream_model in names:
            ...
            try:
                ...
                _hb_every = _stream_heartbeat_seconds(plan.provider)
                _hb_last = time.perf_counter()
                while not collect_task.done():
                    if _pending_stream_chunks:
                        while _pending_stream_chunks:
                            yield _pending_stream_chunks.pop(0)
                        _hb_last = time.perf_counter()
                    elif (time.perf_counter() - _hb_last) >= _hb_every:
                        yield _jsonl({"type": "status", "heartbeat": True,
                                      "content": f"Model is thinking... ({int(time.perf_counter() - started)}s)"})
                        _hb_last = time.perf_counter()
                    await asyncio.sleep(0.02)
                ...
                if lm_equivalent in lm_names_fb:
                    ...
                    async def collect_lm_fb_reasoning(delta: str) -> None:
                        nonlocal lm_fb_reasoning_buffer
                        lm_fb_reasoning_buffer += delta
                        if len(lm_fb_reasoning_buffer) >= 120 or "\n" in delta:
                            pending_lm_fb.append(_jsonl({"type": "thought", "stage": "reasoning", "content": lm_fb_reasoning_buffer[:500]}))
                            lm_fb_reasoning_buffer = lm_fb_reasoning_buffer[500:]
                    ...
                    try:
                        ...
                        collect_task = asyncio.create_task(_collect_lmstudio_stream(
                            ...
                            on_reasoning=collect_lm_fb_reasoning,
                        ...
                        while pending_lm_fb:
                            yield pending_lm_fb.pop(0)
                        if lm_fb_reasoning_buffer.strip():
                            yield _jsonl({"type": "thought", "stage": "reasoning", "content": lm_fb_reasoning_buffer.strip()[:500]})
                            lm_fb_reasoning_buffer = ""
                        if lm_answer.strip():
                            trust = build_trust(
        ...
        if stream_model in lm_names:
            ...
            try:
                ...
                async def collect_lm_reasoning(delta: str) -> None:
                    nonlocal lm_reasoning_buffer
                    lm_reasoning_buffer += delta
                    if len(lm_reasoning_buffer) >= 120 or "\n" in delta:
                        pending_lm_chunks.append(_jsonl({"type": "thought", "stage": "reasoning", "content": lm_reasoning_buffer[:500]}))
                        lm_reasoning_buffer = lm_reasoning_buffer[500:]
                ...
                collect_task = asyncio.create_task(_collect_lmstudio_stream(
                    ...
                    on_reasoning=collect_lm_reasoning,
                ))
                while not collect_task.done():
                ...
                while pending_lm_chunks:
                    yield pending_lm_chunks.pop(0)
                if lm_reasoning_buffer.strip():
                    yield _jsonl({"type": "thought", "stage": "reasoning", "content": lm_reasoning_buffer.strip()[:500]})
                    lm_reasoning_buffer = ""
                if not answer.strip():
...
async def _safe_brain_stream(req: ChatRequest) -> AsyncGenerator[bytes, None]:
    # Casual chat should NOT go through the heavy agent loop by default.
    # Only auto-enable agent mode for explicit slash commands or messages that
    # clearly need local tools (shell/code/build/web/search/file reads).
    ...
    if os.getenv("SHIMS_GRAPH_AGENT", "").strip().lower() in {"1", "true", "yes", "on"}:
        try:
            async for chunk in _brain_graph_stream(req):
                yield chunk
        except asyncio.CancelledError:
            raise
        except Exception as exc:
            ...
            yield _jsonl({"type": "token", "content": f"\n\nGraph agent stream failed: {msg}. Falling back to legacy stream."})
            # Fall through to legacy stream so the user is never stuck
            try:
                ...
                msg2 = str(exc2)[:260] or exc2.__class__.__name__
...
async def _brain_graph_stream(req: ChatRequest) -> AsyncGenerator[bytes, None]:
    """Graph-based agent stream (Phase 2).

    Enabled when SHIMS_GRAPH_AGENT=1. Uses the LangGraph-style state machine
    in shared.agent_graph instead of the monolithic _brain_stream.
    """
    session_id = req.session_id or str(uuid.uuid4())
    started = time.perf_counter()
    ...
    if provider == "ollama" and not model:
        model = _preferred_local_model(await _ollama_models_raw(), realtime=False) or DEFAULT_OLLAMA_MODEL
    elif not model:
    ...
    # Chat runner: thin wrapper around existing LLM helpers
    async def chat_runner(messages: list[dict[str, Any]]) -> dict[str, Any]:
        tools: list[dict[str, Any]] = []
        result, prov, mdl = await agent_loop._chat_with_fallback(
            messages, tools, provider, model, temperature=0.2, timeout=60.0
        )
        return result

    # Tool runner: synchronous wrapper
    def tool_runner(name: str, args: dict[str, Any], sid: str) -> dict[str, Any]:
        return agent_tools.run_tool(name, args, allow_gated=False, session_id=sid)

    graph = AgentGraph()
    graph.set_chat_runner(chat_runner)
    graph.set_tool_runner(tool_runner)

    # Meta event
    meta_trust = build_trust(route="agent:graph", evidence=[], missing_evidence=[], requested_level="draft")
    yield _jsonl({
        "type": "meta",
        "session_id": session_id,
        "model": model,
        ...
        "route": "agent:graph",
        "agent": "graph",
        "brain": f"graph-v1+{BRAIN_VERSION}",
        "conversation_mode": conversation_enabled,
        **_trust_fields(meta_trust),
    })
    ...
    try:
        async for event in graph.run(state):
            if event.get("type") == "token":
                ...
                # Also store answer for conversation history
            if event.get("type") == "done":
                event["session_id"] = session_id
                event["model"] = model
                event["provider"] = provider
                event["route"] = "agent:graph"
            yield _jsonl(event)
    except Exception as exc:
        msg = str(exc)[:260] or exc.__class__.__name__
        log_event("turn.graph_exception", route="graph-error", provider=provider, model=model, ok=False, message=msg, metadata={"session_id": session_id})
        trust = build_trust(route="graph-error", evidence=[], missing_evidence=[f"Graph stream failed: {msg}"], requested_level="unverified")
        if not final_answer:
            yield _jsonl({"type": "token", "content": f"Graph agent failed: {msg}"})
        yield _jsonl({"type": "error", "error": msg, "session_id": session_id, "route": "graph-error", **_trust_fields(trust)})
        yield _jsonl({"type": "done", "session_id": session_id, "model": model, "provider": provider, "route": "graph-error", **_trust_fields(trust)})
        return
    ...
    log_event("turn.graph_done", route="agent:graph", provider=provider, model=model, ok=True, latency_ms=latency_ms, metadata={"session_id": session_id})
...
async def health() -> dict[str, Any]:
    ...
    return {
        ...
        "capabilities": {
            ...
            "voice_profiles": True,
            "amd_gpu_acceleration": get_amd_accelerator().available,
            "amuse_video": bool(amuse_video_status().get("model")),
            "comfyui_image": bool(comfy_ui_status().get("ok")),
        },
        "amd_gpu": get_amd_accelerator().health(),
        "amuse": amuse_video_status(),
        "comfyui": comfy_ui_status(),
    }
...
async def chat_models() -> dict[str, Any]:
    ...
    default_model = local_model or (_preferred_local_model(list(names), realtime=True) if names else DEFAULT_OLLAMA_MODEL)
    return {"ok": True, "default": default_model, "selected_provider": local_provider if local_model else "ollama", "providers": list(PROVIDER_DEFAULTS), "installed": installed, "recommended": rec, "models": installed, "cloud": cloud, "all_cloud": all_cloud, "all_recommended": all_rec, "aliases": _ollama_aliases_payload(), "show_all": show_all}
...
@app.get("/api/amuse/status")
async def amuse_status() -> dict[str, Any]:
    """Report AMUSE/AMD video generation availability."""
    from shared.amd_acceleration import amuse_video_status
    return amuse_video_status()
...
@app.get("/api/amd/gpu")
async def api_amd_gpu() -> dict[str, Any]:
    """AMD GPU acceleration status — DirectML, VRAM, bundle path."""
    from shared.amd_acceleration import get_amd_accelerator
            ...
            async with httpx.AsyncClient(timeout=None) as client:
                async with client.stream("POST", f"{OLLAMA_HOST}/api/pull", json={"model": model, "stream": True}) as r:
                    ...
                    async for line in r.aiter_lines():
                        ...
                        yield _jsonl(obj)
            if not had_error:
                _invalidate_model_list_cache("ollama")
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                for _ in range(3600):  # up to ~1 hour for very large models
                    ...
                    if state in ("completed", "success", "done"):
                        _invalidate_model_list_cache("lmstudio")
                        yield _jsonl({"type": "done", "model": model, "status": f"Installed {model}"})
                        return
...
async def brain_status_endpoint() -> dict[str, Any]:
    from shared.amd_acceleration import amuse_video_status, comfy_ui_status, get_amd_accelerator
    return {
        **omni_brain_status(),
        "background_interval_seconds": BRAIN_BACKGROUND_INTERVAL_SECONDS,
        "amd_gpu": get_amd_accelerator().health(),
        "amuse_ready": bool(amuse_video_status().get("model")),
        "comfy_ready": bool(comfy_ui_status().get("ok")),
    }
...
def _kick_task_drain(n: int = 5) -> None:
    try:
        _spawn_background(asyncio.to_thread(brain_drain_tasks, n))
    except RuntimeError:
        pass
...
async def api_settings_models() -> dict[str, Any]:
    # Settings page is tuned for this laptop: LM Studio is the default local path.
    # Query LM Studio with a longer timeout because its API can take a couple of
    # seconds to enumerate downloaded models, and run it concurrently with a quick
    # Ollama probe so the page never waits on an unused provider.
