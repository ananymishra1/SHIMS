...
async function forceLocalMode(model){
  ...
  setText('#t-model', state.selectedModel.slice(0,22)); setText('#t-route','lmstudio');
...
async function sendText(text, source='typed'){
  ...
  try{
    const payload = {
      ...
      // Let the backend decide: it auto-enables agent mode for slash commands
      // and tool-like requests. Forcing true here pushed every simple chat
      // ("hi", "how are you?") through the slow agent loop.
      agent_mode:false,
      source,
    ...
    await streamNDJSON('/brain/turn', payload, ch => {
      ...
      if(ch.type === 'meta'){
        ...
        addThoughtLine(ch.stage, ch.content, {elapsed: ch.elapsed, elapsed_ms: ch.elapsed_ms, total_ms: ch.total_ms, model: ch.model, provider: ch.provider});
        ...
        addThoughtLine(ch.stage || 'status', ch.content || ch.status || 'Working...', {isStatus:true});
...
function addThoughtLine(stage, content, opts={}){
  if(!stage || !content) return;
  setThinkDot(stage);
  if(!_activeThinking) startThinkingBlock(null);
  const body = _activeThinking && _activeThinking.querySelector('.tt-body');
  if(!body) return;
  const stageLabel = ({plan:'PLAN',conversation:'CONV',context:'CTX',tool:'TOOL',generate:'GEN',status:'·',agent:'AGENT',synthesis:'SYNTH',research:'RSRCH',automation:'AUTO',memory:'MEM'})[stage] || String(stage).toUpperCase();
  const line = document.createElement('div');
  line.className = 'tt-line' + (stage==='status' || opts.isStatus ?' tt-status':'');
  let timing = '';
  if(opts.elapsed_ms !== undefined && opts.elapsed_ms > 0){
    timing = `<span class="tt-time" title="elapsed">${escapeHtml(opts.elapsed)}</span>`;
  } else if(opts.total_ms !== undefined && opts.total_ms > 0 && !opts.isStatus){
    timing = `<span class="tt-time" title="total">${escapeHtml(opts.elapsed || String(Math.round(opts.total_ms/100)/10)+'s')}</span>`;
  }
  let modelTag = '';
  if(opts.model){
    const short = String(opts.model).split('/').pop().slice(0,18);
    modelTag = `<span class="tt-model" title="${escapeHtml(opts.provider||'')} / ${escapeHtml(opts.model)}">${escapeHtml(short)}</span>`;
  }
  line.innerHTML = `<span class="tt-stage">${escapeHtml(stageLabel)}</span><span class="tt-text">${escapeHtml(content)}</span>${timing}${modelTag}`;
  body.appendChild(line);
  _thinkingCount++;
  // Update header with step count and total thinking time so far
  const cnt = _activeThinking.querySelector('.tt-count');
  if(cnt) cnt.textContent = _thinkingCount + (_thinkingCount===1?' step':' steps');
  const head = _activeThinking.querySelector('.tt-head');
  if(head && opts.total_ms !== undefined){
    let totalBadge = head.querySelector('.tt-total');
    if(!totalBadge){
      totalBadge = document.createElement('span');
      totalBadge.className = 'tt-total';
      head.appendChild(totalBadge);
    }
    const sec = (opts.total_ms / 1000).toFixed(2);
    totalBadge.textContent = `Thought for ${sec}s`;
  }
...
async function loadSttModels(){
  ...
  try{
    ...
    const st=document.getElementById('stt-model-status'); if(st) st.textContent='Active: '+(String(d.active).split(/[\\\/]/).pop())+' · Larger = more accurate. Switch applies on your next spoken command..';
...
function ensureSettingsEnhancements(){
  ...
  card.innerHTML=`<h3>🎙 Voice & Conversation</h3>
  <div class="v9-row">
    <button class="v9-btn" onclick="checkSttHealth()">Check STT</button>
    <button class="v9-btn" onclick="speakText('Hello, SHIMS voice is ready and listening.')">Test Speak</button>
    <button class="v9-btn" onclick="loadModelList()">Refresh Models</button>
  </div>
  <div class="v9-row">
    <select id="v9-voice-lang"><option value="en-IN">Indian English (en-IN)</option><option value="hi-IN">Hindi / Hinglish (hi-IN)</option><option value="en-US">English US (fallback)</option></select>
    <button class="v9-btn" onclick="saveVoiceConfig()">Save Voice</button>
  </div>
  <small>Say “Hey SHIMS” to wake. Server STT runs locally for privacy.</small>
  <h3 style="margin-top:16px">🖼 Media Generation</h3>
  <div class="v9-row">
    <select id="v11-image-backend"><option value="auto">Auto</option><option value="pollinations">Pollinations.ai (free, no key)</option><option value="stable-diffusion">Stable Diffusion WebUI API</option><option value="diffusers">Diffusers Local</option><option value="openai">OpenAI Image</option><option value="qwen">Qwen / Alibaba Image</option></select>
    <input id="v11-sd-url" placeholder="STABLE_DIFFUSION_URL e.g. http://127.0.0.1:7860">
    <button class="v9-btn" onclick="saveMediaSettings()">Save Media</button>
  </div>`;
