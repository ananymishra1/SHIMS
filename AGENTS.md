...
## v19 — SHIMS Omni App Factory: J K Hospital

SHIMS Omni can now generate and host large vertical apps. The first example is `apps/jk_hospital/`.
...
- `apps/<app_name>/services/` — domain modules (patients, lab, rooms, ot, ivf, reminders, demo, ai, voice, auth).
...
### Hospital app capabilities
- End-to-end patient journey: inquiry → registry → OPD → diagnosis → tests → treatment → IPD/room → OT → follow-up.
- Role-based default logins (reception/doctor/nurse/lab/ot/ivf/admin), password `JKH2025!`.
- Voice entry in Hindi/English via `POST /hospital/api/voice/transcribe` (faster-whisper + STT corrector).
- AI clinical support: differential diagnosis, treatment suggestions, junior-doctor mentor, IVF cycle insights (local Qwen2.5-Coder-14B).
- IVF module: couple profile, cycle tracking, stimulation, follicle scans, embryo tracking.
- Dashboards + demo data generator + staff estimator.

### Running the hospital app
1. Start the main SHIMS backend.
2. Open SHIMS Omni and click **J K Hospital** in the left modules panel, or navigate to `http://<host>/hospital`.
3. Log in with any default username (e.g. `reception`) and password `JKH2025!`.

### Tests
- `tests/test_jk_hospital.py` covers patient registry, visit summary, vitals/complaints/diagnoses/prescriptions, lab, beds, OT, IVF, demo generator, and staff estimator.

### Extending the factory
- Copy `apps/jk_hospital/` to `apps/<new_app>/`.
...
**Usage from chat:**
- "Diagnose the Stanford school app" → runs `app_factory.diagnose_app`.
- "Repair the Stanford school app" → runs `app_factory.repair_app` (safe, app-directory only).

**Usage from UI/scripts:**
- `POST /api/app-factory/diagnose {"app_name":"stanford_school"}`
- `POST /api/app-factory/repair {"app_name":"stanford_school"}`
